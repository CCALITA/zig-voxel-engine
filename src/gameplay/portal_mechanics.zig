...
/// Returned (inside an optional) only when a teleport fires.
/// `target_dim`: 1 = nether, 2 = end.
...
/// All 12 end portal ring offsets (relative to centre).
const end_ring_offsets = [12][2]i32{
    // North side (z = -2)
    .{ -1, -2 }, .{ 0, -2 }, .{ 1, -2 },
    // South side (z = +2)
    .{ -1, 2 }, .{ 0, 2 }, .{ 1, 2 },
...
test "updatePortalTimer — teleport targets end dimension" {
    var state = PortalState{ .portal_type = .end, .timer = 3.9 };
...
test "checkEndPortal — complete 12-frame ring passes" {
    const valid = checkEndPortal(endRingGetN(12), 0, 64, 0);
    try std.testing.expect(valid);
}

test "checkEndPortal — 10 frames still passes" {
    const valid = checkEndPortal(endRingGetN(10), 0, 64, 0);
    try std.testing.expect(valid);
}

test "checkEndPortal — 9 frames fails" {
    const valid = checkEndPortal(endRingGetN(9), 0, 64, 0);
    try std.testing.expect(!valid);
}
...
/// Returns END_PORTAL_FRAME for the first `count` ring positions, air otherwise.
fn endRingGetN(comptime count: u32) *const fn (i32, i32, i32) u16 {
    return &struct {
        fn get(x: i32, _: i32, z: i32) u16 {
            for (end_ring_offsets[0..count]) |off| {
