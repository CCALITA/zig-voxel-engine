...
pub fn build(b: *std.Build) void {
    ...
    // Creeper entity tests
    const creeper_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/entity/creeper.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    ...
    // Status effect manager tests
    const status_effect_manager_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/gameplay/status_effect_manager.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    const run_status_effect_manager_tests = b.addRunArtifact(status_effect_manager_tests);

    // Water renderer tests
    const water_renderer_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/renderer/water_renderer.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    const run_water_renderer_tests = b.addRunArtifact(water_renderer_tests);

    // Combat system tests
    const combat_system_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/gameplay/combat_system.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    const run_combat_system_tests = b.addRunArtifact(combat_system_tests);

    // FOV effects tests
    const fov_effects_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/gameplay/fov_effects.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    const run_fov_effects_tests = b.addRunArtifact(fov_effects_tests);
